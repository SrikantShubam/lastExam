...
const Login: React.FC = () => {
  ...
  // Function to update or initialize user doc with streak
  const updateLoginStreak = useCallback(async (uid: string) => {
    const userRef = doc(db!, "users", uid);
    const userSnap = await getDoc(userRef);
    const user = auth!.currentUser;

    if (!userSnap.exists()) {
      // Create new user doc if it doesn't exist (e.g., first Google signup)
      await setDoc(userRef, {
        name: user?.displayName || 'User',
        email: user?.email || '',
        avatarUrl: user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.displayName || 'User')}`,
    ...
    let newStreak = 1;
    if (data.lastLoginDate === getYesterdayDate()) {
  ...
  }, []); // Empty deps since db and auth are stable
  ...
  useEffect(() => {
    const handleRedirectResult = async () => {
      try {
        const result = await getRedirectResult(auth!);
        if (result) {
          await updateLoginStreak(result.user.uid); // Update streak after redirect
          router.push("/"); // Redirect to homepage after successful login
        }
      } catch (err: unknown) {
        setState((prev) => ({
    ...
    handleRedirectResult();
  }, [router, updateLoginStreak]);

  const handleLogin = async (values: z.infer<typeof loginSchema>) => {
    setState((prev) => ({ ...prev, error: "", loading: true }));
    try {
      const userCredential = await signInWithEmailAndPassword(auth!, values.email, values.password);
      await updateLoginStreak(userCredential.user.uid); // Update streak
      router.push("/");
    } catch (err: unknown) {
      setState((prev) => ({
        ...prev,
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (isMobile) {
        // Use signInWithRedirect for mobile devices (streak handled in redirect effect)
        await signInWithRedirect(auth!, googleProvider!);
      } else {
        // Use signInWithPopup for desktop devices
