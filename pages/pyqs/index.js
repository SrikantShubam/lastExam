    ...
    const fetchExams = async () => {
      ...
      try {
        ...
        for (const examDoc of examsSnapshot.docs) {
          ...
          // Check if it's a mixed-bag exam with "Full Exam" subcollection
          const mixedBagCheck = await getDocs(collection(db, `exams/${examName}/Full Exam`));
          const isMixedBag = !mixedBagCheck.empty;
          console.log(`${examName} isMixedBag:`, isMixedBag);

          if (isMixedBag) {
            // Fetch papers from "Full Exam" for mixed-bag exams like CLAT
            console.log(`Fetching papers for ${examName}/Full Exam...`);
            const papersSnapshot = await getDocs(collection(db, `exams/${examName}/Full Exam`));
            console.log("Full Exam papers snapshot size:", papersSnapshot.size);

            papersSnapshot.forEach((paperDoc) => {
              const paperName = paperDoc.id;
              if (paperName.toLowerCase() !== "quizzes") {
                allPaperNames.push(`Full Exam/${paperName}`);
                totalPapers += 1;
              }
            });
          } else {
            // Original logic for subject-specific exams
            for (const subject of uniqueSubjects) {
              console.log(`Fetching papers for ${examName}/${subject}...`);
              try {
                const papersSnapshot = await getDocs(collection(db, `exams/${examName}/${subject}`));
          ...
          examsData.push({
            examName: examName,
            subjects: isMixedBag ? ["Full Exam"] : metadataSubjects,
            totalPapers: totalPapers,
          });
        }
