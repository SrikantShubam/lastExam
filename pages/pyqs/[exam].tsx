...
//                   <Link href={`/pyqs/${pyq.exam}/${pyq.subject}/${pyq.paperName}`} className="mt-6 block relative z-10">
...
// mixed bag wherein exams have mixed subjects 
...
// export default function ExamPyqsPage() {
//   const router = useRouter();
//   const { exam } = router.query; // e.g., "NDA" or "CLAT"
//   const [pyqs, setPyqs] = useState([]);
//   const [subjects, setSubjects] = useState([]);
//   const [filters, setFilters] = useState({ subject: "all" });
//   const [loading, setLoading] = useState(true);
...
//       setLoading(true);
//       try {
//         // Step 1: Fetch the exam document
//         const examDocRef = doc(db, "exams", exam);
//         const examDoc = await getDoc(examDocRef);
//         if (!examDoc.exists()) {
//           console.error(`${exam} document not found`);
...
//         // Step 2: Check for mixed-bag exams (e.g., CLAT with "Full Exam" subcollection)
//         const mixedBagCheck = await getDocs(collection(db, `exams/${exam}/Full Exam`));
//         setIsMixedBag(!mixedBagCheck.empty);
//         console.log(`${exam}  isMixedBag:`, !mixedBagCheck.empty);
//         console.log('this is where the user attempts the exam ')

//         if (isMixedBag) {
//           // Handle mixed-bag exam (e.g., CLAT)
//           const papersSnapshot = await getDocs(collection(db, `exams/${exam}/Full Exam`));
//           console.log("Full Exam papers snapshot size:", papersSnapshot.size);
//           if (!papersSnapshot.empty) {
//             foundSubjects.push("Full Exam");
//             papersSnapshot.forEach((paperDoc) => {
//               const paperName = paperDoc.id;
//               console.log("Processing paper:", paperDoc.id, paperDoc.data());
//               if (paperName.toLowerCase() !== "quizzes") {
//                 const paperData = paperDoc.data();
//                 pyqsData.push({
//                   exam: exam,
...
//               }
//             });
//             console.log("pyqsData after Full Exam processing:", pyqsData);
//           }
//         } else {
//           // Handle subject-specific exam (e.g., NDA)
//           for (const subject of metadataSubjects) {
//             const papersSnapshot = await getDocs(collection(db, `exams/${exam}/${subject}`));
//             console.log(`Papers in ${subject}:`, papersSnapshot.size);
//             if (!papersSnapshot.empty) {
//               foundSubjects.push(subject);
//               papersSnapshot.forEach((paperDoc) => {
//                 const paperName = paperDoc.id;
...
//             }
//           }
//           console.log("pyqsData after subject-specific processing:", pyqsData);
//         }

//         setSubjects(foundSubjects);
//         setPyqs(pyqsData);
//         console.log("Subjects set to:", foundSubjects);
//         console.log("pyqs set to:", pyqsData);
//       } catch (error) {
//         console.error("Error fetching PYQs:", error);
//       } finally {
//         setLoading(false);
...
//               move: { enable: true, random: true, speed: 0.3, direction: "none", outModes: { default: "bounce" } },
...
export default function ExamPyqsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPyqs = async () => {
      ...
      try {
        // Step 1: Fetch the exam document
        const examDocRef = doc(db!, "exams", exam as string);
        const examDoc = await getDoc(examDocRef);
        if (!examDoc.exists()) {
          console.error(`${exam} document not found`);
          setLoading(false);
