...
import { doc, getDoc, setDoc, updateDoc } from "firebase/firestore"; // Added Firestore imports
import { auth, googleProvider, db } from "../lib/firebase";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
...
const Signup: React.FC = () => {
  ...
  const handleSignup = async (values: z.infer<typeof signupSchema>) => {
    ...
    try {
      ...
      await updateProfile(userCredential.user, { displayName: values.name });
      // Create/update user doc and streak (will create since new)
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      if (isMobile) {
        // Use signInWithRedirect for mobile (streak handled in redirect effect)
        await signInWithRedirect(auth, googleProvider);
      } else {
